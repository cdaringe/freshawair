...
let log_format_from_str = (function
  | "json" -> `Json
  | "plain" -> `Plain
  | str -> raise (Invald_log_format (Printf.sprintf "invalid log level found in env: %s" str))
  )


let active_log_format = Sys.getenv_opt "LOG_FORMAT" |> function
...
let log ~(level: int) ?(transport=Console.log) (message: string) =
  log_message_to_yojson ({
    timestamp = Core.Time.to_string @@ Core.Time.now ();
    message;
    level;
